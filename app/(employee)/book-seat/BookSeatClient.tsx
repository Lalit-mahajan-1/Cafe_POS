...
export default function BookSeatClient({ user, isAdmin }: BookSeatClientProps) {
  // ── State ──────────────────────────────────────────────────────────────────
  ...
  const handleCellClick = (row: number, col: number) => {
    ...
    if (viewMode === "view" || !isAdmin) return;
  ...
  const submitBooking = async () => {
    if (!bookingTableId || !isAdmin) return;
  ...
  // ── Force complete order (active status change for occupied seats) ──────────
            ...
            >
              {Array.from({ length: rows }).map((_, rowIndex) =>
                Array.from({ length: cols }).map((__, colIndex) => {
                  ...
                  return (
                    <div key={`${row}-${col}`} className="relative group/tooltip aspect-square min-h-11">
                      <button
                        type="button"
                        ...
                        className={`w-full h-full rounded-lg border text-xs font-bold transition-all duration-200 cursor-pointer ${getCellStyles(
                          table,
                          isSelected
                      ...
                      {/* Tooltip for occupied seats on hover */}
                      {table && table.status === "OCCUPIED" && table.activeOrder && (
                        <div className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-56 -translate-x-1/2 scale-95 rounded-lg border border-[#705C53] bg-[#000505] p-3 text-left text-xs text-[#FDFBF7] opacity-0 transition-all duration-150 group-hover/tooltip:scale-100 group-hover/tooltip:opacity-100 shadow-xl">
                          <div className="font-bold text-[#C86446] mb-1.5 border-b border-[#705C53]/40 pb-1 flex items-center gap-1.5">
                            <Coffee className="w-3.5 h-3.5" />
                            Occupied Table details
                          </div>
                          <div className="space-y-1">
                            <p>
                              <span className="text-[#F3EFE8]/60 text-[10px] block">Order ID:</span>
                              <span className="font-mono text-[9px] break-all bg-[#705C53]/15 px-1 py-0.5 rounded text-[#F3EFE8]/90">
                                {table.activeOrder.id}
                              </span>
                            </p>
                            <p className="flex justify-between items-center text-[10px] pt-0.5">
                              <span className="text-[#F3EFE8]/60">Order No:</span>
                              <span className="font-semibold text-emerald-400">{table.activeOrder.orderNumber}</span>
                            </p>
                            <p className="flex justify-between items-center text-[10px]">
                              <span className="text-[#F3EFE8]/60">Order Total:</span>
                              <span className="font-bold">₹{table.activeOrder.total.toFixed(2)}</span>
                            </p>
                          </div>
                          <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-1 border-4 border-transparent border-t-[#000505]" />
                        </div>
                      )}
                    </div>
                  );
        ...
        <aside className="space-y-6">
                  ...
                  <>
                    ...
                    {selectedTable.activeOrder && (
                      <div className="rounded-lg border border-red-200 bg-red-50 p-4 space-y-3">
                        <div>
                          ...
                          <div className="space-y-1.5 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-mono text-[10px] text-red-800 break-all select-all bg-red-100/50 px-1 rounded">
                        ...
                        {/* Force Complete button - Admin only */}
                        {isAdmin && (
                          <button
                            onClick={() => handleForceComplete(selectedTable.activeOrder!.id)}
                            disabled={isCompleting}
                            className="flex w-full items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-lg text-xs font-semibold transition-all duration-200 cursor-pointer disabled:opacity-60"
                          >
                            {isCompleting ? (
                    ...
                    {/* Book this table button — Admin only */}
                    {isAdmin && (selectedTable.status === "AVAILABLE" || !selectedTable.status) && (
                      <button
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <label className="block">
                        ...
                        <input
                          ...
                          min={1}
                          max={rows}
                          value={selectedTable.row}
                          onChange={(e) =>
                            updateSelectedTable("row", Math.min(rows, Math.max(1, Number(e.target.value))))
                          }
                          className="w-full px-4 py-2.5 bg-[#FDFBF7] border border-[#E8E0D8] rounded-lg text-sm text-[#000505] focus:outline-none focus:border-[#C86446] focus:ring-2 focus:ring-[#C86446]/15 transition-all duration-200"
                        />
                      </label>
                      <label className="block">
                        ...
                        <input
                          ...
                          min={1}
                          max={cols}
                          value={selectedTable.col}
                          onChange={(e) =>
                            updateSelectedTable("col", Math.min(cols, Math.max(1, Number(e.target.value))))
                          }
                          className="w-full px-4 py-2.5 bg-[#FDFBF7] border border-[#E8E0D8] rounded-lg text-sm text-[#000505] focus:outline-none focus:border-[#C86446] focus:ring-2 focus:ring-[#C86446]/15 transition-all duration-200"
                        />
                      </label>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className={`${fraunces.className} text-base font-bold text-[#000505]`}>
                {viewMode === "edit" && isAdmin ? "Assigned Tables" : "All Tables"}
              </h2>
