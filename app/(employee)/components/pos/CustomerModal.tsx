...
export default function CustomerModal({
    ...
    const customersWithPhone = useMemo(() => existingCustomers.filter((customer) => Boolean(customer.phone)), [existingCustomers]);
    ...
    const filteredCustomers = useMemo(() => {
        ...
        return customersWithPhone.filter((customer) => normalizePhone(customer.phone ?? "").startsWith(query));
            ...
            >
                <div className="mb-5 flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-[#705C53]">Search an existing customer or create a new one.</p>
                    </div>
                ...
                <div className="mb-5 grid grid-cols-2 gap-2 rounded-xl bg-[#F3EFE8] p-1">
                    <button
                        ...
                        className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${
                            mode === "lookup" ? "bg-[#1F1815] text-[#ffffff] shadow-sm" : "text-[#1F1815] hover:text-[#705C53]"
                        }`}
                    >
                        Existing
                    </button>
                    ...
                    <button
                        ...
                        className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${
                            mode === "new" ? "bg-[#1F1815] text-[#ffffff] shadow-sm" : "text-[#1F1815] hover:text-[#705C53]"
                        }`}
                    >
                        New
                    </button>
                    ...
                    <div className="space-y-3">
                        ...
                        >
                            ...
                            <div className="relative">
                                ...
                                {loading && <Loader2 className="absolute right-3 top-1/2 size-4 -translate-y-1/2 animate-spin text-[#705C53]" />}
                            </div>
                                        ...
                                        {filteredCustomers.length > 0 ? (
                                            filteredCustomers.map((customer, index) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <p className="text-sm font-semibold text-[#1F1815]">{customer.phone}</p>
                                                        <p className="mt-0.5 text-xs text-[#705C53]">{customer.name}</p>
                                                    </button>
                                                );
                                            })
                                        ) : (
                                            <div className="px-4 py-6 text-center">
                                                <p className="text-sm font-medium text-[#1F1815]">No customers found</p>
                                                <p className="mt-1 text-xs text-[#705C53]">Try another phone number or switch to New.</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        <p className="text-xs text-[#705C53]">Click a result to assign instantly. You can also use ↑ ↓ and Enter.</p>
                    </div>
                ) : (
                    <form
                        className="space-y-3"
